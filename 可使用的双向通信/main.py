...
class LoginWindow(QMainWindow):
    ...
    def login_in(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Failed to send JSON data: {str(e)}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # self.ui.pushButton_up.clicked.connect(lambda: self.move('UP'))
        # self.ui.pushButton_left.clicked.connect(lambda: self.move('LEFT'))
        # self.ui.pushButton_right.clicked.connect(lambda: self.move('RIGHT'))
        # self.ui.pushButton_down.clicked.connect(lambda: self.move('DOWN'))
        self.ui.pushButton_up.clicked.connect(lambda: self.move('up'))
        self.ui.pushButton_left.clicked.connect(lambda: self.move('left'))
        self.ui.pushButton_right.clicked.connect(lambda: self.move('right'))
        self.ui.pushButton_down.clicked.connect(lambda: self.move('down'))
    ...
    def move(self, direction):
        ...
        try:
            send = requests.post("http://123.57.236.111:801/direction", json=info)
            send.raise_for_status()  # 检查响应状态码，如果不是200会抛出异常
